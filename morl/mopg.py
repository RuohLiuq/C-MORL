...
def MOPG_worker(args, task_id, task, device, iteration, num_updates, start_time, results_queue, done_event):
    ...
    for j in range(start_iter, final_iter):
        ...
        if j <=3500:
            ...
            max_kl = 1e-5
            d_k = -2.5
            value_loss, action_loss, dist_entropy = agent.constraint_update(rollouts, damping, d_k, max_kl, scalarization = None, obj_var = None, use_fim = True)
        ...
        if args.rl_log_interval > 0 and len(episode_rewards) > 1:
            ...
            #             end - start_time),'rewards:',np.mean(print_reward[:,0]),np.mean(print_reward[:,1]),np.mean(print_reward[:,2]),\
            #                 'std:',np.std(print_reward[:,0]),np.std(print_reward[:,1]),np.std(print_reward[:,2]),scalarization.weights)
            print(
                "[RL] Updates {}, num timesteps {}, FPS {}, time {:.2f} seconds"
                .format(j + 1, total_num_steps,
                        int(total_num_steps / (end - start_time)),
