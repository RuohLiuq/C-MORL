...
parser.add_argument('--save-dir', type=str, default='./results/fruit-tree-v2')
...
for i in range(args.num_seeds):
    ...
    if test_cmorl_ipo:
        cmd = f'python morl/run.py '\
            f'--env-name fruit-tree-v0 --obj-num 6 '\
            f'--seed {seed} '\
            f'--num-time-steps 500000 '\
            f'--num-init-steps 100000 '\
            f'--ref-point -1 -1 -1 -1 -1 -1 '\
            f'--min-weight 0 '\
            f'--max-weight 1 '\
            f'--delta-weight 1 '\
